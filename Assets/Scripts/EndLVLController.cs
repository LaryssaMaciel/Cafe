...
public class EndLVLController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (_deliveryController.contEntregue >= _taskController.minEntregas && !player.gameOver)
        {
    ...
    public void EndScreen(int x, bool b) // tela de pause/endgame
    {
        Time.timeScale = x; // pausa/despausa
        panel.SetActive(b); // mostra/esconde painel
        if (this.taskCompleted && end && !player.gameOver) // se completou task e chegou no fim da fase
        {
        ...
        else if (!this.taskCompleted && end || player.gameOver) { btnResume.SetActive(false); } // esconde btn resume
    }
